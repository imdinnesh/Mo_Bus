...
func VerifyToken(tokenString string) (string, uint, string, string, error) {
	...
	if redis.IsTokenBlacklisted(userID, deviceID, tokenString) {
...
func RefreshAccessToken(refreshToken string) (string, string, error) {
	...
	decryptedRefreshToken, err := crypto.DecryptToken(refreshTokenEntry.EncryptedRefreshToken)
	if err != nil {
	...
	newAccessToken, err := CreateToken(email, newUser.ID, deviceID, role)
	if err != nil {
	...
	encryptedRefreshToken, err := crypto.EncryptToken(newRefreshToken)
	if err != nil {
