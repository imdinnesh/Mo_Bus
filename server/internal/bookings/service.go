...
type Service interface {
	...
	GetBookingByQuery(bookingID string,userID uint) (*GetBookingsResponse, error)
}
...
func (s *service) GetBookingByQuery(bookingID string,userID uint) (*GetBookingsResponse, error) {
	booking, err := s.repo.GetBooking(bookingID,userID)
	if err != nil {
		return nil, apierror.New("Failed to get booking", http.StatusInternalServerError)
	}
