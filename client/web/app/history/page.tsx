import { CalendarDays, MapPin, ArrowRight, IndianRupee, Clock, Filter } from "lucide-react";
import {
    Table,
    TableBody,
    TableCaption,
...
export default async function HistoryPage() {
    ...
    const uniqueRoutes = new Set(
        bookings.map(
            (booking) => `${booking.start_stop_name} to ${booking.end_stop_name}`
        )
    ).size;
    const mostRecentBooking = bookings.length > 0
    ...
    // Group bookings by route for summary
    ...
    const monthlySpending = Object.entries(bookingsByMonth).map(([month, monthBookings]) => ({
        month,
